...
file_path = 'data/API_LP.LPI.OVRL.XQ_DS2_en_excel_v2_6299222.xls'

lpi_ovr_data = pd.read_excel(file_path)
...
filtered_lpi_ovr_data = lpi_ovr_data[lpi_ovr_data['Country Name'].isin(asian_countries)]

# Melting the dataframe to get a long format with Country, Code, Year, and Value
melted_lpi_ovr_data = filtered_lpi_ovr_data.melt(id_vars=['Country Name', 'Country Code'],
                                         var_name='Year',
                                         value_name='LPI Overall')

from constant import years_to_keep, countries_to_keep, years_to_keep_s
melted_lpi_ovr_data = melted_lpi_ovr_data[melted_lpi_ovr_data['Year'].isin(years_to_keep + years_to_keep_s)]
melted_lpi_ovr_data = melted_lpi_ovr_data[melted_lpi_ovr_data['Country Name'].isin(countries_to_keep)]
# Saving the transformed dataframe to a new Excel file
transformed_file_path = 'cleaned/Transformed_LPI_OVERALL.xlsx'
melted_lpi_ovr_data.to_excel(transformed_file_path, index=False)

print(melted_lpi_ovr_data.head())
